...
def descenso_gradiente(
    ...
    hipotesis: Callable[[Any], Any],  # Sigmoide
    # [[List[float], List[float], List[float], float], List[float]], # Callable[[parametros], resultado]
    gradiente: Callable[[Any], Any],
    max_iters: int = 10000,
    alpha: float = 0.0001,
    _lambda: float = 0.0,
    grado: int = 1
    ...
    # [1] ya que X viene en formato de filas, por lo que cada columna es una observacion.
    unos = np.ones(x_set.shape[1])
    ...
    # retorno X, ya que incluye la col de uno's, la cual será útil en cross validation.
    return tetas
